...
# TAB 1: ACCUEIL - DASHBOARD ÉTENDU
with tabs[0]:
    page_header(f"Dashboard - {m_nom} {a_sel}", f"Compte de {user_actuel}")
    ...
    # Préparation des données sur 12 mois
    ...
    with col_graph1:
        ...
        fig1.update_layout(
            title="Revenus vs Dépenses (12 mois)",
            height=300,
    ...
    st.markdown("### Comparaisons & Tendances")
    ...
    # Prédiction simple (tendance linéaire sur 3 mois)
    ...
    tab_ana1, tab_ana2, tab_ana3 = st.tabs(["Carte de Chaleur", "Top Dépenses", "Tendances par Catégorie"])
    
    with tab_ana1:
        st.markdown("#### Quand dépensez-vous le plus ?")
        
        # Préparation données carte de chaleur
        ...
        if not df_user_mois.empty:
            ...
            semaines.append(heatmap_data.iloc[28:])  # Derniers jours
            ...
            st.info("Pas assez de données pour générer la carte de chaleur")
    ...
    with tab_ana3:
        ...
        if len(categories_dep) > 0:
            ...
            st.info("Aucune catégorie de dépense disponible")
